...
for i in range(2, 25):
    ...
    if not model.check_if_model_exists(f"generated_models/comparison/{model_name}.h5"):
        model.train_model(output_file_name=model_name, save_file=True, model_path='comparison', epochs=5)
    else:
        model.load_model(f"generated_models/comparison/{model_name}")
    model.evaluate_model(model_name=model_name, log_evaluation=True, log_path='comparison', include_classification_metrics=False)
    ...
    number_of_neurons.append(network_layers[2][1])
